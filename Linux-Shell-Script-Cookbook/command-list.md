...
圆括号中的命令会在子shell执行，命令都会有执行结果，当在圆括号前加上美元符$, 说明需要获取命令的执行结果.
...
(4)其它类型的特殊字符，比如()定义匹配的子串，|表示二选一，\表示转义。
...
其实可能我们想关注的是整体的磁盘使用情况，可以重点关注最后一列Mounted on。
有一行Mounted on是在根路径下，即/。那么是否意味着这个就是系统磁盘的总的一个使用情况了。
```
...
```
命令本质上也是一段程序，执行命令就是调用别人写好的程序。至于命令内部如何实现调用者一般不怎么关心(linux中原生命令大多数都是c或c++实现的)。

当我们需要测试一段程序或比较不同的算法时，执行时间是一个很重要的指标。

好的算法执行时间应该比较短。
```

```
专门用来统计命令执行时间的命令是time命令。

time命令又分为两种，一种是shell自带内建的time命令，一种是第三方的time命令，二进制可执行文件一般位于/usr/bin/time。无特别指明默认情况下使用的应该是shell自带的time命令，第三方的time可以通过绝对路径来使用。

两者的区别在于shell自带的time命令选项没那么多，也就是提供的功能没有第三方的那么丰富。
...
output:
total 20
drwxr-xr-x. 3 root root 4096 Aug  8 02:25 .
...
(2)user time: 指目标进程花费在用户模式下(内核模式之外)的CPU时间。这是唯一真正用于执行进程所花费的CPU时间。执行其它进程以及花费在IO等阻塞状态中的时间并没有计算在内。

(3)sys time: 指目标进程花费在内核模式下的CPU时间。它表示目标进程在内核中执行系统调用所使用的时间。和user time类似，这是真正用于目标进程的CPU时间，只不过user time是用户态CPU的使用时间，sys time是内核态CPU的使用时间。


follow up:
CPU有两种状态，一个是内核态，一个是用户态。不同状态下CPU的权限不一样，内核态下的CPU拥有更高的权限。


回到上面的例子，根据三种不同类型的时间定义，可以肯定real time肯定时间长度是大于另外两个的，因为它包括了其它杂七杂八的时间，即:
real time > user time, 
real time > sys time。

上面的例子还碰巧：
real time = user time + sys time
但是这个等式只是碰巧成立，因为real time包括了其它杂七杂八的时间，等于说明没有其它进程抢占了cpu时间，以及没有阻塞时间等。
所以，有了该结论:
real time >= user time + sys time.
